...
%load fmincon_fh_90s_horizon2_post_done3.mat
...
X11(1,:) = x_ic;
for i=1:size(U1,1)
    X11(i + 1,:) = rk4_integrate(X11(i,:)', U1(i,:)', dt, 1);
end
...
X4(1,:) = x_ic;
for i=1:size(U1,1)
    X4(i + 1,:) = rk4_integrate(X4(i,:)', U1(i,:)', dt, 2);
end
...
X1(1,:) = x_ic;
for i=1:size(U1,1)
    X1(i + 1,:) = rk4_integrate(X1(i,:)', U1(i,:)', dt, 4);
end
...
X5(1,:) = x_ic;
for i=1:size(U1,1)
    X5(i + 1,:) = rk4_integrate(X5(i,:)', U1(i,:)', dt, 8);
end
...
X13(1,:) = x_ic;
for i=1:size(U1,1)
    X13(i + 1,:) = rk4_integrate(X13(i,:)', U1(i,:)', dt, 16);
end
