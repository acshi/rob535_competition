...
d_alpha_r_d_v = -u/(u^2 + (v - br)^2);
d_alpha_r_d_u = (v - b*r)/(u^2 + (v - b*r)^2);
d_alpha_r_d_r = b*u/(u^2 + (v - b*r)^2);

alpha_f = delta_f - atan((v + a*r)/u);
d_phi_d_alpha_f = (1 - E_y) + E_y/(1 + (By*(alpha_f*pi/180 + Sny))^2);
alpha_r = -atan((v - b*r)/u);
d_phi_d_alpha_r = (1 - E_y) + E_y/(1 + (By*(alpha_r*pi/180 + Sny))^2);

phi_r = (1 - Ey)(alpha_r + Shy) + (Ey/By)*atan(By*(alpha_r + Shy));
d_Fyr_d_phi = Fzr*Dy*cos(Cy*atan(By*phi_alpha_r))*Cy*By/(1 + (By*ph_r)^2);
...
if Ftotal > Fmax
    n = Fmax/Ftotal;
    dn_dFx = -0.5*Fmax*2*Nw*Nw*Fx((Nw*Fx)^2 + (Fyr)^2)^(-1.5);
    dn_dFyr = -0.5*Fmax*2*Fyr((Nw*Fx)^2 + (Fyr)^2)^(-1.5);
else
    n = 1;
    dn_dFx = 0;
    dn_dFyr = 0;
...
d_Fsyr_d_r = d_Fsyr_d_Fyr*dFyr_d_r;
...
A(u_ind, u_ind) = (1/m)*(Nw*d_Fsx_dFyr*d_Fyr_d_u - d_Fyf_d_u*sin(df));
