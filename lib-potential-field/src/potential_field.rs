...
fn multiply_actual(a : Vec<f64>, b : Vec<f64>) -> Vec<f64> {
    //if a.len() != b.len() {
...
#[test]
fn test_multiply_actual() {
    let a = vec![1f64, 2f64, 3f64];
    let b = vec![3f64, 2f64, 1f64];
    let c = multiply_actual(a, b);
    let expected = vec![3f64, 4f64, 3f64];
...
pub extern fn multiply(a_double: *mut c_double,
    ...
    let c: Vec<f64> = multiply_actual(a,b);
